...
abstract class BaseDotsIndicator @JvmOverloads constructor(
    ...
    fun setRecyclerView(recyclerView: RecyclerView, scrollView: HorizontalScrollView) {
        ...
        pager = object : Pager {
            ...
            override fun addOnPageChangeListener(onPageChangeListenerHelper: OnPageChangeListenerHelper) {
                if (VERSION.SDK_INT >= VERSION_CODES.M) {
                    val l = OnScrollChangeListener { _, scrollX, scrollY, oldScrollX, oldScrollY ->
                        Log.i(
                            "onScrolled",
                        ...
                        if (recyclerView.adapter!!.itemCount > 0) {
                            ...
                            val position = scrollX / itemWidth
                            outCurrentItem = position
                            val positionOffset = (scrollX.toFloat() % itemWidth) / itemWidth
                            Log.i("onScrolled", "positionOffset = $positionOffset")
                            Log.i("onScrolled", "position = $position")
                            onPageChangeListenerHelper.onPageScrolled(position, positionOffset)
                        }
                    }
...
//                                val itemWidth = (layoutManager.getChildAt(0)?.width ?: 0)
//                                val positionOffset = Math.abs(totallDx) % itemWidth / itemWidth
//                                Log.i("onScrolled", "positionOffset = $positionOffset")
////                            Log.i("onScrolled", "totallDx = $totallDx")
////                            Log.i("onScrolled", "totallDx = $totallDx")
//                                onPageChangeListenerHelper.onPageScrolled(position, positionOffset)
//                            }
//                        }
